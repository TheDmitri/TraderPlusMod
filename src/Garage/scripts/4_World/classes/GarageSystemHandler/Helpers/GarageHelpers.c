...
class GarageHelpers
{
	...
	static bool CreateVehicle(PlayerBase player, string classname, int cargoId, int collateralMoney, int id, float health, float fuel, vector position, vector orientation, array<ref ItemsCargo> items)
	{
		...
		#endif
		EntityAI ent = EntityAI.Cast(GetGame().CreateObject(classname, vector.Zero, false, false, true));
		if (ent)
		{
			ent.SetPosition(position + Vector(0, 0.3, 0));
			ent.SetOrientation(orientation);
      vector roll = ent.GetOrientation();
      roll[2] = roll[2] - 1;
      ent.SetOrientation(roll);
      roll[2] = roll[2] + 1;
      ent.SetOrientation(roll);
			...
			if(GetGarageConfig().SaveVehicleCargo)
			{
				if(cargoId != 0)
				{
					...
					if(TraderPlusVehicleCargo.GetMapAll().Find(cargoId, vehicleCargo))
					{
