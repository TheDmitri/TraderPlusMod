class NewReceiptBase extends ItemBase
{
	...
	override void OnPlacementComplete( Man player, vector position = "0 0 0", vector orientation = "0 0 0" )
	{
			...
			if ( GetGame().IsServer() )
			{
					...
					pposition[1]=pposition[1]+TraderPlusHelper.GetHeightIfClassNameInVehicleConfig(CarClassName);
					vector oorientation = player_base.GetLocalProjectionOrientation();

					GetInventory().UnlockInventory(HIDE_INV_FROM_SCRIPT);
					SetSynchDirty();

					TraderPlusHelper.CreateVehicle(player_base,CarClassName,pposition,oorientation, Attachments, this, CarUniqueId, CollateralMoney, CarPassword);
