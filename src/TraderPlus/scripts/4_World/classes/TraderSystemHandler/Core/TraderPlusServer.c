class TraderPlusServer
{
      ...
      foreach(int idx, TraderPlusIDs tpId: IDs)
      {
        foreach(string category: tpId.Categories)
        {
          ...
          foreach(int jdx, string product: tpStockCategory.Products)
          {
            ...
            GetTraderPlusLogger().LogInfo("ID:" + tpId.Id + "look for " + token[0] + " in priceconfig");
    ...
    {
      GetTraderPlusLogger().LogInfo("SetTradersSpawner: Started");
      ...
      for(int i = 0;i<Traders.Count();i++)
      {
        string objetName = Traders.Get(i).Name;
    		...
    		if (!obj)
        {
          GetTraderPlusLogger().LogInfo("obj was not created: "+objetName + " please make sure the syntaxe is correct!");
          continue;
        }
      ...
      GetTraderPlusLogger().LogInfo("SetTradersSpawner: Ended");
    }

    void SetObjectSpawner()
    {
      GetTraderPlusLogger().LogInfo("SetObjectSpawner: Started");
      ...
      GetTraderPlusLogger().LogInfo("SetObjectSpawner: Ended");
    }
    ...
    bool SellRequest(TraderPlusProduct product)
    {
      ...
      if(state)
      {
        ...
        GetTraderPlusLogger().LogInfo("the stock doesn't contain our item so we can add it to our stock");
        TraderPlusCategory tpStockCategory = TraderPlusHelper.GetStockCategory(product.TraderID,product.Category, false);
    ...
    {
      GetTraderPlusLogger().LogInfo("InsuranceRequestHandler");
